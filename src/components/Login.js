...
const Login = ({setUser,user}) => {
    ...
    const handelGoogleLogOut =() =>{
        signOut(auth,googleProvider)
        .then(()=> setUser({}))
    }
    return (
        <div className='p-5'>
            {user.uid ? <button onClick={handelGoogleLogOut} type="button" className="px-8 py-3 font-semibold rounded bg-gray-500 m-3 text-gray-800">Log Out</button> : 
                ...
                <div className="my-6 space-y-4">
                    <button onClick={handleGooleLogin} aria-label="Login with Google" type="button" className="flex items-center justify-center w-full p-4 space-x-4 border rounded-md focus:ring-2 focus:ring-offset-1 border-gray-400 focus:ring-violet-400">
